...
func TestSyncUpdatesStateFile(t *testing.T) {
	...
	if _, ok := st.Skills["agents:alpha"]; !ok {
		t.Error("expected agents:alpha in state")
	}
	if _, ok := st.Skills["agents:beta"]; !ok {
		t.Error("expected agents:beta in state")
	}
...
func TestSyncDoesNotSkipNewToolWhenSkillUnchanged(t *testing.T) {
	sourceDir, destDirs, cleanup := setupSyncTest(t)
	defer cleanup()
	...
	// Add claude as a new configured tool and sync again without --all.
	...
	claudeDest := destDirs[paths.ToolClaude]
	if _, err := os.Stat(filepath.Join(claudeDest, "alpha", "SKILL.md")); err != nil {
		t.Error("alpha should be copied to newly added claude tool")
	}
	...
	if _, ok := st.Skills["claude:alpha"]; !ok {
		t.Error("expected claude:alpha in state")
	}
}
