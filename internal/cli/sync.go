...
func skillStateKey(tool paths.Tool, skillName string) string {
	return fmt.Sprintf("%s:%s", tool, skillName)
}
...
func runSync(opts *syncOptions) error {
	...
	for _, tool := range toolList {
		...
		if opts.dryRun {
			for _, skill := range selected {
				...
				prev, hasPrev := st.Skills[skillStateKey(tool, skill.Name)]
	...
	if !opts.dryRun {
		...
		syncedSkillByTool := map[string]bool{}
		for _, r := range results {
			for _, name := range r.synced {
				syncedSkillByTool[skillStateKey(paths.Tool(r.tool), name)] = true
			}
		}
		if len(syncedSkillByTool) > 0 {
			now := config.NowTimestamp()
			st.LastSync = now
			for key := range syncedSkillByTool {
				parts := strings.SplitN(key, ":", 2)
				name := parts[1]
				st.Skills[key] = config.SkillState{
